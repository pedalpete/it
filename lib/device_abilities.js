...
var favor_obj;
try {
    favor_obj = JSON.parse(fs.readFileSync(config_file,'utf8'));
} catch(e) {
...
parsedObject.prototype = {
    init: function(el){
        //if no element passed, get the entire favor_obj (device description)
        if(!el) return favor_obj;
    ...
    filterComponents: function(el,count){
        ...
        for(var i=0; i<favor_obj.components.length; i++){
            if(els.test(favor_obj.components[i].type)) {
                 matches.push(favor_obj.components[i]);
            }
            // stops incrementing if the correct number of components is in the array
            if(matches.length===count) i=favor_obj.components.length;
        }
