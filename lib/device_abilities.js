var parseObject = function(el, favor_obj){
    return new parsedObject(el, favor_obj);   
};
function parsedObject(el, favor_obj){
    this.favor_obj = favor_obj
    return this.init(el);
...
parsedObject.prototype = {
    ...
    filterComponents: function(el,count){
        ...
        for(var i=0; i<this.favor_obj.components.length; i++){
            if(els.test(this.favor_obj.components[i].type)) {
                 matches.push(this.favor_obj.components[i]);
            }
            // stops incrementing if the correct number of components is in the array
            if(matches.length===count) i= this.favor_obj.components.length;
        }
