...
function getI2Ccmd(cmp, callback) {
	var cmd = I2CCMDs[cmp.type] === 'write' ? 'writeByte' : 'readByte';
	return cmd;
}
...
function i2c(i2cCmd, callback) {
	...
	var action = getI2Ccmd(i2cCmd);
	if (/^read/.test(action)) return _fvr[this._index].i2c[action](address, i2cCmd.addr, i2cCallback);
	if (!this._valueToSet) newVal = i2cCmd.cmd;
	if (!newVal) newVal = this._valueToSet;
	_fvr[this._index].i2c[action](address, i2cCmd.addr, newVal, i2cCallback);
}
...
module.exports = function() {
	...
	if (!_fvr[fvr._index].i2c) {
		_fvr[fvr._index].i2c = I2C.openSync(_fvr[fvr._index].path);
	}
