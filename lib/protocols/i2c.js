...
function I2C(i2cCmd, callback){
    ...
    if(!i2cCmd.bytes) return _fvr[this._index].i2c[getI2Ccmd(i2cCmd)](i2cCmd.byte, function(err, i2cVal){
        if(err) fvr.err;
        if(i2cCmd.wait){
    ...
    var newVal;  
    if(typeof i2cCmd.bytes === 'function') newVal = i2cCmd.bytes.call(fvr, this._valueToSet);
    if(!this._valueToSet) newVal = i2cCmd.bytes;
    if(!newVal) newVal = this._valueToSet;
    _fvr[this._index].i2c[getI2Ccmd(i2cCmd)](i2cCmd.byte, newVal, function(err, i2cVal){
        if(err) fvr.err;
		if(i2cCmd.wait){
        	return setTimeout(function(){
				callback(err, i2cVal);
...
function returnCallback(i2cVal) {
	var fvr = this;
	if (_fvr[fvr._index].post_action) {

		return fvr._callback.call(fvr, _fvr[fvr._index].post_action.call(fvr, i2cVal));
	}
...
module.exports = function() {
	...
	if(_fvr[fvr._index].address===undefined) return fvr.onError("i2c bus address must be specified");
	var path = _fvr[fvr._index].i2c_path || _fvr.i2c_path || '/dev/i2c-1';
	
	if(!_fvr[fvr._index].i2c) _fvr[fvr._index].i2c = new i2c(_fvr[fvr._index].address,{device: path, debug: false});
	
	if(!_fvr[fvr._index].init){
		_fvr[fvr._index].initialized = true; //no init sequence needed
		return runI2CMethod.call(fvr);
	}
