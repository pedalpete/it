...
function getI2Ccmd(cmp, callback) {
	var cmd = I2CCMDs[cmp.type] === 'write' ? 'writeBlock' : 'readByte';
	return cmd;
}
...
function i2c(i2cCmd, callback) {
	...
	if (i2cCmd.type === 'read') return _fvr[this._index].i2c.readByte(address, i2cCmd.addr, i2cCallback);
	if (!this._valueToSet) newVal = i2cCmd.cmd;
	if (!newVal) newVal = this._valueToSet;
	_fvr[this._index].i2c.writeByte(address, i2cCmd.addr, newVal, i2cCallback);
}
