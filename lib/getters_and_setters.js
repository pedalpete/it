...
function initializeComponent(val,next,cb){
    switch(this.component.interface || this.parent.interface) {
        case 'i2c':
            initializeI2C.call(this,val,next,cb);
            break;
        case 'gpio':
            if(!this.component.direction){
                this.component.direction = this.parent.direction || DIRECTION_OBJ[this.parent.type];
            }
                this.component.gpio = gpio.connect(this.component.address);
                this.component.gpio.mode(this.component.direction); //set direction
            this.component.initialized=true;
            if(next==='get') get.call(this,cb);
            if(next==='set') set.call(this,val,cb);
            if(next==='change') this.component.gpio.on(val,cb);//set the on change event to respond as the callback
            break;
    }
}
...
function get(cb){ // return gpio value
    var fvr = this;
    if(this.component.get) return this.component.get.call(fvr,function(val){
        if(fvr.component.link)return cb.call(fvr,val[fvr.component.return_as]);
        cb.call(fvr, val); 
    });
    switch(this.component.interface) {
        case 'gpio' :
            cb.call(this,this.component.gpio.value());
            break;
        case 'i2c' :
            i2c_get(this.component);
            break;
    }
}
...
function set(val,cb){ //set gpio value
    var fvr = this;
    if(this.component.set) return this.component.set.call(fvr,val,function(){
        cb.call(fvr);   
    });
    switch(this.component.interface) {
    case 'gpio' :
        val = checkGPIOVal.call(this,val);
        this.component.gpio.value(val);        
        if(cb) cb.call(this);
        break;
    }
}
...
*/
  for(var i = 0; i< this.length; i++){
        ...
        if(this.component.structure){
            var keys = getKeysToUpdate(this.component.structure,key);
            for(var k=0;k<keys.length;k++){
                this.component = this.component.structure[keys[k]];
...
function getComponentAsObject(cb){
    ...
    for(var key in this.structure){
        this.component = this.structure[key];
        if(!this.component.initialized){
...
var _get = function(cb){
    for(var i=0; i<this.length; i++){
        setup_fvr.call(this,i);
        if(this.component.structure){ //not directly addressed, this is a group of  I/Os  
          return getComponentAsObject.call(cb);  
        } else {
           	if(!this.component.initialized && this.component.interface === 'gpio'){
...
function addOnchange(dir, fn){
    ...
    var fvr = this;
    if(['both','rise','fall'].indexOf(dir)<0) return this.onError.call(fvr,'invalid value for watching changes. Valid watches are both, rise, fall',200, file);
	for(var i=0;i<this.length;i++){
		 if(this[i].structure){
             this.component=this[i].structure;
             this.parent=this[i];
            var keys = Object.keys(this.component.structure);
            for(var k=0;k<keys.length;k++){
                this.component=components[i].structure[keys[k]];
                if(!this.component.initialized){
                     initializeComponent.call(this,dir,'change',fn);   
                }else {
                    this.component.gpio.on(dir,fn);
                }
            }
        } else {
            this.component=this[i];
