...
function initializeComponent(val,next,cb){
    switch(_fvr[this._index].interface || _fvr[this._parent].interface) {
        ...
        case 'gpio':
            ...
            if(typeof next === 'object') onData.call(this, next ,cb);//set the on change event to respond as the callback
            break;
    }
}
...
function initializeI2C(val, cb){
    ...
    if(loopNeeded.call(this)) return waterfall.call(this, val, cb);
    return I2C.call(this, val, cb);
}
...
function I2C(val, cb){
    var fvr = this;
    if(fvr._method.event) {
        delete fvr._method;
        return onData.call(fvr, val, cb);
    } 
    var cmp = this._method_count!==undefined ? _fvr[this._index][this._method][this._method_count] : _fvr[this._index][this._method];
    if(!cmp){
        var err = 'error calling '+this._method+', on favorit object at index '+this._index;
        this.onError(err); 
        if(cb) cb.call(fvr, {err: err});
        return;
    }
    
    if(!cmp.bytes) return _fvr[this._index].i2c[i2ccmd(cmp)](cmp.byte, function(err,i2cVal){
        if(err) fvr.err;
        if(!fvr._next) return postDevice.call(fvr,i2cVal,cb);
 ...
 function afterWrite(val, cb){
     var fvr = this,
      ...
      if(this.err) this.onError('error writing i2c', err);
        if(this._method_count!==undefined){
            wait = _fvr[this._index][this._method][this._method_count].wait;
            this._method_count++;
        }
        if(this._method==='init' && !loopNeeded.call(this)){
            _fvr[this._index].initialized = true;
            this._method = this._next;
            delete this._next;
...
function get(cb){
    ...
    switch(_fvr[this._index].interface) {
        case 'gpio' :
            postDevice.call(this,_fvr[this._index].gpio.value(),cb);
            break;
        case 'i2c' : 
            fvr._method='get';
...
function findPostAction(){
    if(_fvr[this._index][this._method] && _fvr[this._index][this._method].post_action) return _fvr[this._index][this._method].post_action;
    return _fvr[this._index].post_action || undefined;
}

function postDevice(val,cb){
    var self = this;
    var post_action = findPostAction.call(self);
    if(post_action && cb) return cb.call(self, post_action.call(self,val));
    if(post_action) return self.post_action,call(self, val);
...
function setStreaming() {
    var fvr = this;
    _fvr[this._index]._eventEmitter = new Events.EventEmitter(); 
...
function onData(watcherObj, cb) {
    if (_fvr[this._index].interface === 'gpio' && watcherObj.watcher === 'change' && typeof watcherObj.change === 'string') return _fvr[this._index].gpio.on(watcherObj.change, cb);
    if(cb.name.length === 0) return this.onError.call(fvr, 'watching stream callbacks must be named functions');
    if (!_fvr[this._index].streamingData) setStreaming.call(this);
    _fvr[this._index]._eventEmitter.on('data', cb);
}


function removeOn(watcher, fn) {
    if(_fvr[this._index]._eventEmitter) _fvr[this._index]._eventEmitter.removeListener(watcher, fn);
}

var _removeOn = function(watcher, fn) {
	for(var i=0;i<this._component_matches.length;i++){
        setup_fvr.call(this,i);
        if(_fvr[this._index].structure){
            for(key in _fvr[this._index].structure){
                this._index = this._index['structure'][key];
                removeOn.call(this,watcher, fn);        
            }
        } else {
          removeOn.call(this, watcher, fn);        
        }
...
function addOn(watcher, change, fn){
    if (['change', 'data'].indexOf(watcher) === -1) return this.onError.call(fvr, 'must pass a valid watch of change or data');
    if(typeof change === 'function' && !fn){
        fn = change;
        change = 'both';
    }
    var fvr = this
    if(watcher === 'change' && ['both','rise','fall'].indexOf(change)===-1) return this.onError.call(fvr,'invalid value for watching changes. Valid watches are both, rise, fall',200, file);
    var watcherObj = {
        event: true,
        watcher: watcher,
        change: change
        ...
        setup_fvr.call(this,i);
		 if(_fvr[this._index].structure){
            ...
            for(key in _fvr[this._index].structure){
                this._index = this._index['structure'][key];
                if(!_fvr[this._index].initialized){
                     return initializeComponent.call(this, change, watcherObj,fn);   
                }else {
                    return onData.call(this, watcherObj, fn);
                }
            }
        } else {   
		  if(!_fvr[this._index].initialized){
...
var _on = function(watcher, dir, fn){ 
    	addOn.call(this, watcher, dir, fn);  
}
