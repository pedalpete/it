...
function initializeI2C(val, cb){
    var i2c_address = _fvr[this._index].i2c_address || this.i2c_address;
    if(!i2c_address) return this.onError("i2c bus address must be specified");
    
    _fvr[this._index].i2c = new i2c(i2c_address, _fvr[this._index].i2c_path || this.i2c_path);
...
function I2C(val, cb){
    ...
    if(!cmp.bytes) return _fvr[this._index].i2c[i2ccmd(cmp)](cmp.byte, function(err,i2cVal){
        ...
        if(!checkArray.call(fvr)) return cb.call(this,i2cVal);
       afterWrite.call(fvr, val, cb);
    });
  
    _fvr[this._index].i2c[i2ccmd(cmp)](cmp.byte, cmp.bytes, function(err,i2cVal){
        if(err) fvr.err;
        if(!checkArray.call(fvr)) return cb.call(this, i2cVal);
        afterWrite.call(fvr, val, cb);
    });
