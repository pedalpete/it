...
function get() {
	var fvr = this;
	if (isFunc.call(this,'get')) return componentUtils.getFromFunc.call(this);
...
function watch() {
	...
	if (!_fvr[this._index].interface) {
		return componentUtils.intervalWatch.call(fvr);
	}
	switch (_fvr[this._index].interface) {
		case 'gpio' :
...
function removeWatch(cb) {
	if (!_fvr[this._index].interface) {
		return componentUtils.removeWatch.call(fvr);
	}
	switch (_fvr[this._index].interface) {
		case 'gpio' :
			gpio.unwatch.call(this, this._callback);
			break;
		case 'i2c' :
			i2c.call(this);
			break;
...
var _watch = function(cb) {
	addWatch.call(this, cb);
};
