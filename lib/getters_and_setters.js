...
function initializeComponent() {
	var fvr = this;
	var interface = _fvr[this._index].interface || _fvr[this._parent].interface;
	switch (interface) {
		case 'i2c':
			i2c.call(this);
			break;
	...
	for (var i = 0; i < this._componentMatches.length; i++) {
		...
		if (this._component.structure) {
			...
			if (!_fvr[this._index].initialized) {
				...
				if(typeof this._method !== 'function') console.log('no method', _fvr[this._index], typeof this._method)
				this._method.call(this);
			}
		}
	}
