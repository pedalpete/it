...
function initializeComponent(){
    ...
    switch(_fvr[this._index].interface || _fvr[this._parent].interface) {
        case 'i2c':
            _fvr[this._index]._eventEmitter.once('initialized', function(){ i2c.call(fvr)});
            i2c.call(this);
            break;            
        case 'gpio':
    ...
    for(var i=0; i<this._componentMatches.length; i++){
        ...
        if(_fvr[this._index].structure){ //not directly addressed, this is a group of  I/Os  
          getComponentAsObject.call(this);  
            ...
            if(!_fvr[this._index].initialized){
            if(_fvr[this._index].interface === 'i2c') _fvr[this._index]._eventEmitter.once('initialized', function(){
...
function removeOn() {
    ...
    try {
       ...
       _fvr[this._index]._eventEmitter.removeListener(this._watch.watcher, this._callback);
    } catch(e) {
        console.log('err', e);
    }
}
   ...
   for(var i=0;i<this._componentMatches.length;i++){
      ...
      addCallbackToObject.call(this, cb);
	   ...
	   if(_fvr[this._index].structure){
		  ...
		  if(!_fvr[this._index].initialized){
            if(_fvr[this._index].interface === 'i2c') _fvr[this._index]._eventEmitter.once('initialized', function() {onData.call(fvr)});
			return initializeComponent.call(this);
          }
