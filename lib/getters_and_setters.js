...
function loopNeeded(){
    ...
    console.log('loop needed', _fvr[this._index][this._method].length>this._method_count, _fvr[this._index][this._method].length, this._method_count);
...
function I2C(val, cb){
    ...
    if(!cmp.bytes) return _fvr[this._index].i2c[i2ccmd(cmp)](cmp.byte, function(err,i2cVal){
        if(err) fvr.err;
        if(!fvr._next && cb) return cb.call(fvr, i2cVal);
        if(!fvr._next) return;
