...
Favor_Obj.prototype.init = function(passed_var){
                if(passed_var){
                    //build the object
                    var psed = this.device_abilities(passed_var, this);
                    for(var i=0; i<psed.length; i++){
                        this[i] = psed[i];   
                    }
                    this.length = psed.length;
                }
                return this;
            }
    ...
    try {
            ...
            */
            ...
            Favor_Obj.prototype.init = function(passed_var){
                if(passed_var){
                    //build the object
                    var psed = this.device_abilities(passed_var);
                    for(var i=0; i<psed.length; i++){
                        Favor_Obj[i] = psed[i];   
                    }
                    this.length = psed.length;
