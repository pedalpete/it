...
module.exports = {
	...
	components : [
		...
		{type: 'link', name: 'rht11', methods: [
			...
			structure: {'temp': {'address':9}, humidity: {address:10}}, interface: 'gpio'},
		...
		{type: 'accelerometer', name: 'init_stream', address: 0x1d, init: [
			...
			{cmd: 'write',byte: 0x2c, bytes: [8 + 2 + 1], wait: 500}], 
			get: {cmd: 'read', byte:0x33, bytes:6}, interface: 'i2c'},
		{type: 'led', name: 'blinkm', address: 0x09, init: {cmd: 'write', byte: 0x6d}, 
		...
		{type: 'accelerometer', name: 'postAction', address: 0x11, init: [
			{cmd: 'write', byte: 0x2D, bytes: [1 << 3]},
			{cmd: 'write', byte: 0x31,bytes: [0x09]},
			{cmd: 'write', byte: 0x2c, bytes: [8 + 2 + 1]}],
			get: {cmd: 'read', byte: 0x33, bytes: 6}, interface: 'i2c', 
				postAction: postFormat},
		{type: 'temperature', name: 'postAction',  address: 9, interface: 'gpio',
			postAction: postFormat},
		{type: 'temperature', name: 'spi', interface: 'spi', address: '/dev/spidev0.0',
			mode: 'MODE_0',	chipSelect: 'high', get: [0x23, 0x48, 0xAF, 0x19, 0x19, 0x19]}
	]
};