...
describe('working with i2c', function() {
	it('should get i2c', function() {
		...
		runs(function() {
			expect(acc).toBe('51');
			expect(mocki2c.write.length).toBe(3);
			expect(mocki2c.write[0]).toBe('45,8');
			expect(mocki2c.write[2]).toBe('44,11');
	...
	it('should wait if a wait value is supplied', function() {
		...
		runs(function() {
			...
			expect(acc).toBe('51');
			expect(mocki2c.write.length).toBe(9);
	...
	it('should use the input value as the bytes value', function() {
		...
		runs(function() {
			expect(mocki2c.write.length).toBe(11); //gets initialized first
			expect(mocki2c.write[9]).toBe('109,0,0,0');
			expect(mocki2c.write[10]).toBe('110,0,0,0');
		});
	});
	...
	it('should take a function as the set value', function() {
		...
		runs(function() {
			expect(mocki2c.write.length).toBe(12);
			expect(mocki2c.write[11]).toBe('110,1,2,3');
		});
	});
	...
	it('should get the correct function after initialized', function() {
		...
		runs(function() {
			expect(mocki2c.write.length).toBe(13);
			expect(mocki2c.write[12]).toBe('110,2,2,2');
		});
	});
